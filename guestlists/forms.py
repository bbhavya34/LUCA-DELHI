...
class GuestEntryForm(forms.ModelForm):
    ...
    def __init__(self,*a,user=None,**kw):
        ...
        self.fields["event"].queryset=Event.objects.none() if not user else Event.objects.filter(status=Event.Status.ACTIVE,assigned_promoters__user=user)
    ...
    def clean(self):
        ...
        if event and (event.status!=Event.Status.ACTIVE or not event.assigned_promoters.filter(user=self.user).exists()): raise forms.ValidationError("This event is not available to you.")
...
class AdminGuestEntryForm(GuestEntryForm):
    class Meta(GuestEntryForm.Meta):
        fields=("promoter",)+GuestEntryForm.Meta.fields
    def __init__(self,*a,**kw):
        kw.pop("user",None)
        forms.ModelForm.__init__(self,*a,**kw)
        self.fields["promoter"].queryset=get_user_model().objects.filter(role="MEMBER",is_account_active=True)
        self.fields["event"].queryset=Event.objects.filter(status=Event.Status.ACTIVE)
        self.fields["pass_type"].queryset=PassType.objects.filter(is_active=True,event__status=Event.Status.ACTIVE)
        [f.widget.attrs.setdefault("class","form-control") for f in self.fields.values()]
    def clean(self):
        d=forms.ModelForm.clean(self); event,pt,promoter=d.get("event"),d.get("pass_type"),d.get("promoter")
        if event and event.status!=Event.Status.ACTIVE: self.add_error("event","Select an active event.")
        ...
        if event and promoter and not event.assigned_promoters.filter(user=promoter).exists(): self.add_error("promoter","This promoter is not assigned to the selected event.")
        if event and pt and pt.event_id!=event.id: self.add_error("pass_type","Pass type does not belong to this event.")
        if d.get("amount_paid") is not None and d["amount_paid"]<0: self.add_error("amount_paid","Amount cannot be negative.")
        return d
