...
from promoters.services import calculate_promoter_commission
...
@admin_required
def qr_list(request): return render(request,"admin_os/payments/qr_list.html",{"page_obj":Paginator(PaymentQRCode.objects.select_related("event"),20).get_page(request.GET.get("page"))})
...
@admin_required
def qr_create(request): return _qr_form(request)
@admin_required
...
def member_qr(request):
    events=request.user.promoter_profile.assigned_events.filter(status="ACTIVE"); event=events.filter(pk=request.GET.get("event")).first() if request.GET.get("event") else events.first(); now=timezone.now()
    qr=PaymentQRCode.objects.filter(event=event,is_active=True).filter(Q(active_from__isnull=True)|Q(active_from__lte=now)).filter(Q(active_until__isnull=True)|Q(active_until__gte=now)).first() if event else None
...
def member_earnings(request):
    ...
    return render(request,"member_os/earnings/earnings.html",{"verified":calculate_promoter_commission(request.user),"pending":calculate_promoter_commission(request.user,status="PENDING"),"breakdown":breakdown,"entries":entries})
